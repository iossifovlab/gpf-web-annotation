...
class CustomWebsocketCommunicator(ApplicationCommunicator):
    """
    ApplicationCommunicator subclass that has WebSocket shortcut methods.

    This class is copied from channels.testing.websocket.WebsocketCommunicator,
    due to its constructor not providing custom scope population support.
    """

    def __init__(
        self, application: Any, path: str, headers: list | None = None,
        subprotocols: list | None = None, spec_version: str | None = None,
        **kwargs: Any,
    ) -> None:
    ...
    async def connect(self, timeout: int = 1) -> tuple[bool, int | str | None]:
        """
    ...
    async def send_to(
        ...
        if text_data:
            assert isinstance(text_data, str), \
                "The text_data argument must be a str"
    ...
    async def send_json_to(self, data: dict) -> None:
        """
        Sends JSON data as a text frame
        """
        await self.send_to(text_data=json.dumps(data))

    async def receive_from(self, timeout: int = 1) -> str | bytes:
        """
    ...
    async def receive_json_from(self, timeout: int = 1) -> dict:
        """
